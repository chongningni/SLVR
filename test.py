...
import numpy as np
import torch
# from data.normal_dataset import DownsampledDataset
...
def test_pipeline(args):
    ...
    # config.lit_model.args.network.args.thresholds = [0, 1, 1]
    ...
    for dataset_name in test_datasets:
        for scale in scales:
            ...
            psnrs, ssims, run_times, flops, sal_p = [], [], [], [], []
            for batch_idx, batch in tqdm(enumerate(dataloader), total=len(dataset)):
                ...
                if "flops" in rslt.keys():
                    flops.append(rslt["flops"])
                if "sal_value_list" in rslt.keys():
                    sal_p.append(rslt["sal_value_list"])
            ...
            mean_flop = np.array(flops).mean()
            if "params" in rslt.keys():
                params = rslt["params"]
            else:
                params = 0
            ...
            print("- FLOPs :    {0} ({1})".format(clever_format(mean_flop), mean_flop))
            print("- Params :   {0} ({1})".format(clever_format(params), params))
            if sal_p:
                print("- SalList:   {0}".format(np.array(sal_p).mean(0).tolist()))
            print("=" * 42)
