...
%% set path
methods = {'SLVR'};
dataset = {'~\results\B100'};
scale_list = 4;
ext = {'*.jpg', '*.png', '*.bmp'};
num_method = length(methods);
num_set = length(dataset);
record_results_txt = 'PSNR_SSIM_Results.txt';
    ...
    for idx_set = 1:num_set
        ...
        for scale = scale_list
            ...
            for idx_ext = 1:length(ext)
                filepaths = cat(1, filepaths, dir(fullfile(dataset{idx_set},'HR', ext{idx_ext})));
            end
            PSNR_all = zeros(1, length(filepaths));
            SSIM_all = zeros(1, length(filepaths));
            parfor idx_im = 1:length(filepaths)
                name_HR = filepaths(idx_im).name;
                name_SR = name_HR;
                im_HR = imread(fullfile(filepaths(idx_im).folder, filepaths(idx_im).name));
                im_SR = imread(fullfile(dataset{idx_set}, methods{idx_method}, ['x', num2str(scale)], name_SR));
                ...
                % calculate PSNR, SSIM
                [PSNR_all(idx_im), SSIM_all(idx_im)] = Cal_Y_PSNRSSIM(im_HR_Y*255, im_SR_Y*255, ceil(scale), ceil(scale));
