...
class SalCSSRModelAblation2(pl.LightningModule):
    """
    Basic SR Model optimized by pixel-wise loss
    Remove psnr loss and saliency loss
    """
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.current_epoch < self.hparams.trainer.pretrain_epochs:
            loss = self.loss_fn(smap, smap_pred)
        else:
            loss_sr_list = []
            for sr_ in sr:
                loss_sr_list.append(
            ...
            # loss_sal = loss_weights[1] * self.loss_fn(smap, smap_pred)
            # loss_psnr = loss_weights[2] * self.loss_fn(psnr_map, smap_pred)
            loss = loss_sr  # + loss_sal  + loss_psnr
        return loss
