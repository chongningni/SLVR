...
class SalCSSRModelAblationBCE(pl.LightningModule):
    """
    ...
    def training_step(self, batch, batch_idx):
        ...
        smap_pred = F.interpolate(
            smap_pred, size=smap.shape[2:], mode="bilinear", align_corners=False
        )
        # print(smap_pred)
        if self.current_epoch < self.hparams.trainer.pretrain_epochs:
            ...
            loss_sal = loss_weights[1] * self.bce_loss(smap_pred, smap)

            # loss_psnr = loss_weights[2] * self.loss_fn(psnr_map, smap_pred)
            loss = loss_weights[0] * loss_sr + loss_sal  # + loss_psnr
        return loss
    ...
    def check_MSE_Map(self, hr, sr):
        ...
        err_f_img = exposure.equalize_hist(err_f)  # 得到直方图均衡化后的图像
