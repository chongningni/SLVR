...
def calc_metrics_hs(img1, img2):

    psnr = PSNR_HS(img1, img2)
    ssim = SSIM_HS(img1, img2)
    sam = SAM_HS(img1, img2)
    return psnr, ssim, sam


# from "Mixed  2D/3D Convolutional Network for Hyperspectral Image Super-Resolution"
...
def SSIM_HS(pred, gt):
    ...
    for i in range(gt.shape[0]):
        ssim = ssim + compute_ssim_hs(pred[i, :, :], gt[i, :, :])
...
def matlab_style_gauss2D(shape=np.array([11, 11]), sigma=1.5):
    ...
    h = np.exp(-(m * m + n * n).astype(np.float32) / (2.0 * sigma * sigma))
    h[h < eps * h.max()] = 0
    sumh = h.sum()
...
def compute_ssim_hs(im1, im2, k1=0.01, k2=0.03, win_size=11, L=1):
